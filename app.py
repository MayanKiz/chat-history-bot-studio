...
with st.sidebar:
    ...
    if not st.session_state.logged_in:
        ...
        if st.button("Logout"):
            ...
            st.rerun()
    st.markdown("---")
    st.markdown("<div class='Sample Message'>Pro tip: manage bots and upload files inside the Manage tab (no sidebar actions required).</div>", unsafe_allow_html=True)
            ...
            with col_side:
                ...
                for b in user_bots:
                    st.markdown(
                        ...
                        f"<b>{b['name']}</b><div class='Sample Message'>{b.get('persona','')}</div></div>",
                        unsafe_allow_html=True
                    )
